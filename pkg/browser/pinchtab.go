...
func NewPinchTabManager(baseURL, token string, actionTimeout time.Duration) *PinchTabManager {
	...
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("GOCLAW_BROWSER_PINCHTAB_MODE")))
	if mode != "headed" && mode != "headless" {
		mode = "headless"
	}
	profileName := strings.TrimSpace(os.Getenv("GOCLAW_BROWSER_PINCHTAB_PROFILE"))
	if profileName == "" {
		profileName = "goclaw"
	}
...
// Start creates a default profile and starts a Chrome instance in PinchTab.
...
func (p *PinchTabManager) startOnceLocked(ctx context.Context) error {
	profileID, err := p.ensureProfileLocked(ctx, p.profileName)
	if err != nil {
		return err
	}
